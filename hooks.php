...
add_hook('after_plugin_install', function ($params) {
    $name = $params['name'] ?? '';
    if ($name != 'cloudreve_group') {
        return;
    }
...
add_hook('after_plugin_uninstall', function ($params) {
    $name = $params['name'] ?? '';
    if ($name != 'cloudreve_group') {
        return;
    }

    Db::execute("DROP TABLE IF EXISTS `idcsmart_module_cloudreve_group_plus_users`");
    Db::execute("DROP TABLE IF EXISTS `idcsmart_module_cloudreve_group_map`");
    Db::execute("DROP TABLE IF EXISTS `idcsmart_module_cloudreve_group_custom_cycle`");
    Db::execute("DROP TABLE IF EXISTS `idcsmart_module_cloudreve_group_custom_cycle_pricing`");
    Db::execute("DROP TABLE IF EXISTS `idcsmart_module_cloudreve_group_host_email`");
});
