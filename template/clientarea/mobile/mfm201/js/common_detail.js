...
const app2 = Vue.createApp({
  ...
  computed: {
    ...
    calcMultilSelectText() {
      const str = [];
      this.refundDialog.reasons
        .filter((is) => this.refundForm.arr.includes(is.id))
        .forEach((item) => {
  ...
  methods: {
    ...
    restoreCascadeSelections(savedData) {
      ...
      this.configoptions.forEach(item => {
        ...
        this.cascadeStates[item.id].selectedIds = ids;
        this.cascadeStates[item.id].selectedItems = items;
        this.cascadeStates[item.id].quantity = quantity || 1;
      });
    },
    ...
    osSelectGroupChange(e) {
      this.osIcon =
        "/plugins/server/cloudreve_group/template/clientarea/mobile/mfm201/img/cloudreve_group/" +
        e[0].id.toLowerCase() +
        ".svg";
      ...
      if (e === "1") {
        ...
        upApi(id)
          .then((res) => {
            ...
            this.selectDuration = res.data.data.upgrade[0]?.custom_cycles[0];
            this.getConfig();
        ...
        if (tabVal === "1") {
          ...
          const obj = this.configoptions.reduce((all, cur) => {
            ...
            if (cur.option_type === "os") {
              ...
              this.osIcon =
                "/plugins/server/cloudreve_group/template/clientarea/pc/default/img/cloudreve_group/" +
    ...
    async changeConfig() {
      ...
      try {
        ...
        if (tabVal === "1") {
          ...
          const cycle = result.data.data.custom_cycles.find((item) => item.id === this.cycle);
          if (cycle) {
            this.selectDuration = cycle;
          }
          // 重新计算周期价格显示
          ...
          const params = {
            configoption: temp1, buy: this.isBuyServe,
            cascade_configoption: this.getAllCascadeResults(),
          };
          res = this.buy_id
    ...
    //           item.base_price = item.current_base_price;
    //           return item;
    //         });
    ...
    //         this.firstDueTime = this.renewPageData[0].due_time || 0;
    ...
    //   this.firstDueTime = item.due_time || 0;
